...
local function worker(args)
	...
	local calendar_themes = {
		...
		dark = {
			...
			focus_date_fg = '#101010',
			weekend_day_bg = '#202020',
			weekday_fg = '#ffffff',
			header_fg = '#ffffff',
			border = '#404040'
		},
...
-- 			local default_bg = (weekday == 0 or weekday == 6) and calendar_themes[theme].weekend_day_bg or calendar_themes[theme].bg
			local default_bg = (weekday == 0 or weekday == 5) and calendar_themes[theme].weekend_day_bg or calendar_themes[theme].bg
		...
		function calendar_widget.toggle()
			...
			if popup.visible then
				...
				elseif placement == 'top_right' then
					awful.placement.top_right(popup, { margins = { top = 34, right = 10}, parent = awful.screen.focused() })
